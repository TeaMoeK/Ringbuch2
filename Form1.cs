...
namespace RBii
{
    public partial class Form1 : Form
    {
        ...
        private void ergebnisseBindingSource_CurrentChanged(object sender, EventArgs e)
        {

        }

        private void btnSetSchFest_Click(object sender, EventArgs e)
        {
            DateTime dt = new DateTime(dtpSchFest.Value.Year, dtpSchFest.Value.Month, dtpSchFest.Value.Day);

            verschiedenesTableAdapter.UpdateSchFest(dt);
            try
            {
                this.Validate();
                this.verschiedenesBindingSource.EndEdit();
                verschiedenesTableAdapter.Update(this.rBDataVerschiedenes.Verschiedenes);
            }
            catch (Exception)
            {
